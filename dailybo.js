...
async function fetchVenueData(venue) {
  ...
  try {
    ...
    const poisoned = data?._poisoned;

    if (authorized === true) {
      console.log(`✅ ${venue.id} → AUTH OK`);
    } else if (authorized === false) {
      console.log(`❌ ${venue.id} → POISONED`);
    }

    if (poisoned) {
      console.log(`💀 ${venue.id} → DATA CORRUPTED`);
    }

    if (data?._warning) {
      console.log(`⚠ ${venue.id}: ${data._warning}`);
    }

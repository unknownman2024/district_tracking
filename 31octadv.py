...
const API_URL = "https://districtvenues1.text2029mail.workers.dev/?cinema_id={cid}&date={date}";
...
if (!WORKER_UA || !WORKER_KEY) {
  console.error("❌ Missing WORKER_UA / WORKER_KEY");
  process.exit(1);
}
...
async function fetchVenueData(venue) {
  ...
  try {
    ...
    const authorized = dbg.authorized;

    if (authorized === true) {
      console.log(`✅ ${cid} → AUTH OK`);
    } else if (authorized === false) {
      console.log(`❌ ${cid} → POISONED RESPONSE`);
    } else {
      console.log(`⚠ ${cid} → NO DEBUG INFO`);
    }

    if (data?._warning) {
      console.log(`⚠ ${cid} warning: ${data._warning}`);
    }
